...
* ### RDB란?
  RDB는 관계형 데이터 베이스이며 모든 데이터를 2차원 테이블 형태로 표시합니다. 또한 각각의 테이블들은 상호관련성(relation - 관계)을 가집니다. (외래키를 이용해 관계를 맺은 다른 테   이블을 조인할 수 있습니다.)
  
* ### 쿠키, 캐시, 세션의 차이

* ### MVC 패턴이란?
  MVC 패턴이란 애플리케이션의 구성요소를 Model, View, Controller로 분리하는 것을 의미합니다. 컨트롤러는 요청을 받거나, 모델에 데이터를 담아서 사용자에게 보이는 뷰(사용자에게 보여   지는 영역)에 전달합니다.
